...
def distance(dot1, dot2): #Функция для поиска расстояние между точками
    a = dot1[0] - dot2[0]#расстояние по абсциссе
    b = dot1[1] - dot2[1]#расстояние по оординате
...
def normal(dot, dot1, dot2): #возвращает расстояние от точки dot до прямой, проходящей, через dot1 и dot2
    a = distance(dot, dot1)#подсчёт расстояний
    b = distance(dot, dot2)#подсчёт расстояний
    c = distance(dot1, dot2)#подсчёт расстояний
    p = 0.5*(a+b+c)
    try:
...
def Sgn(dot, dot1, dot2): #Если треугольник из вершин в аргуменете функции Sgn остроугольный или прямоуг, либо тупой угол находится при вершине dot, то вернёт 1 
    a = distance(dot, dot1)
    b = distance(dot, dot2)
    c = distance(dot1, dot2)
    lst = []#пустой список
    try:
...
def Intersec(dot1, dot2, dot3, dot4): #Если отрезки пересекаются или совпадают, то вернёт 1 иначе -1
    x1=dot1[0]
    y1=dot1[1]
    x2=dot2[0]
    y2=dot2[1]
